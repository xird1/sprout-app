...
<button type="button" onClick={() => this.props.history.push("/employees/index")} className="btn btn-primary">Back</button>
  ...
  async getEmployee(id) {
    ...
    const response = await fetch('api/employees/' + id, {
      ...
      const date = new Date(data.birthdate).toISOString().split('T')[0]
      this.setState({ id: data.id, fullName: data.fullName, birthdate: date, tin: data.tin, employeeTypeId: data.employeeTypeId, loading: false,loadingSave: false });
  }
}
